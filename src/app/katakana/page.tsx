...
export default function KatakanaPage() {
  ...
  const currentChar = katakanaData[currentIndex]

  useEffect(() => {
    // Load mastery levels
    const levels: Record<string, number> = {}
    katakanaData.forEach(char => {
      levels[char.character] = progressTracker.getMasteryLevel('katakana', char.character)
    })
    setMasteryLevels(levels)
  }, [])

  const handleAnswer = (correct: boolean) => {
    progressTracker.updateCharacterProgress('katakana', currentChar.character, correct)
    setMasteryLevels(prev => ({
      ...prev,
      [currentChar.character]: progressTracker.getMasteryLevel('katakana', currentChar.character)
    }))
    ...
    setTimeout(() => {
      ...
      setUserInput('')
      setShowAnswer(false)
      if (currentIndex < katakanaData.length - 1) {
  ...
  const previousCharacter = () => {
    if (currentIndex > 0) {
      ...
      setCurrentIndex(katakanaData.length - 1)
    }
    setUserInput('')
    setShowAnswer(false)
    setFeedback(null)
  ...
  const overallProgress = Object.values(masteryLevels).filter(level => level >= 70).length
  const progressPercentage = (overallProgress / katakanaData.length) * 100

  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-bold">Katakana Learning ア</h1>
        <p className="text-lg text-gray-600">
          Learn Katakana characters used for foreign words and names
        </p>
        <div className="flex justify-center space-x-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{katakanaData.length}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose how you want to practice Katakana</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Character {currentIndex + 1} of {katakanaData.length}</span>
            <Badge 
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Katakana Characters</CardTitle>
